...
class Tenant(models.Model):
    company = models.ForeignKey(Company,on_delete=models.CASCADE,related_name="tenants")
    ...
    def __str__(self):
        return f"{self.user.full_name} - {self.property.name}"
...
class RentPayment(models.Model):
    ...
    def __str__(self):
        return f"{self.tenant.user.full_name} - {self.amount}"
...
class Announcement(models.Model):
    company = models.ForeignKey(Company,on_delete=models.CASCADE)
    property = models.ForeignKey(Property,on_delete=models.CASCADE)
    created_by = models.ForeignKey(CompanyStaff,on_delete=models.CASCADE)
    title = models.CharField(max_length=255)
...
class CompanyWallet(models.Model):
    ...
    def __str__(self):
        return f"{self.company.name} Wallet"
